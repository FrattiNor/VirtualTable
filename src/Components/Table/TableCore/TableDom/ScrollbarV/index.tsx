...
type Props<T> = Pick<TableInstance<T>, 'v_scrollbar' | 'bordered' | 'vScrollbarRef' | 'bodyRef' | 'getV_virtualCore' | 'v_totalSize'>;

const ScrollbarV = <T,>(props: Props<T>) => {
	const { throttle } = useFrameThrottle();
	const { v_scrollbar, bordered, vScrollbarRef, getV_virtualCore, bodyRef, v_totalSize } = props;
	...
	if (v_scrollbar.have && v_scrollbar.width === 0) {
		return (
			<div ref={vScrollbarRef} className={styles['v-scrollbar-absolute']}>
				<div className={styles['v-scrollbar-absolute-inner']} style={{ height: v_totalSize }} />
			</div>
		);
	}
